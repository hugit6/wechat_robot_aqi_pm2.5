...
def get_detail():
    payload = {'id': os.getenv('LASEREGGID')}
    try:
...
def send_to_chatroom(pm2_5, pm10, receive_time, room_name='MYROOM'):
...
def tuling_auto_reply(uid, msg):
        if True:
            ...
            body = {'key': os.getenv('AIR_TULINGID'), 'info': msg.encode('utf8'), 'userid': user_id}
	    try:
...
while(1):
    ...
    if reportcount==0:
        detail_data = get_detail()
        send_to_chatroom(detail_data[0],detail_data[1],detail_data[2],"airtest")
    time.sleep(1800)
